...
export default function Set({ sets, setSets }){
    ...
    const handleDelete = () => {
        if(window.confirm("Are you sure you want to delete this set?")){
            let updatedSets = sets.filter(curSet => curSet.id !== numberId);
            navigate('/');
            setSets(updatedSets);
        }
    }
            ...
            <div className="flashcard-container">
                <div onClick={()=>{setIsFlipped(!isFlipped)}} className={`flashcard ${isFlipped ? 'flipped' : ''} `}>
                    <div className="card-front">
                        <div id="termText">{set.flashcards[index].term.trim()}</div>
                    </div>
                    <div className="card-back">
                        <div id="definitionText" style={{display: set.flashcards[index].definition ? "block" : "none"}}>{set.flashcards[index].definition.trim()}</div>
                ...
                <progress value={index} max={set.flashcards.length-1}></progress>
